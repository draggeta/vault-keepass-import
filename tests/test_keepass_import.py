...
def verify_withattachment(vault_server, kv_version):
    ...
    if kv_version == "2":
        entry = client.secrets.kv.v2.read_secret_version("keepass/withattachment")[
            "data"
        ]["data"]
    else:
        entry = client.secrets.kv.v1.read_secret("keepass/withattachment")["data"]
...
def switch_to_kv_v1(vault_server):
    ...
    client.sys.enable_secrets_engine(
        backend_type="kv", options={"version": "1"}, path="secret/"
    )
...
def test_vault_secret_operators(vault_server, version):
    ...
    client = hvac.Client(url=vault_server["http"], token=vault_server["token"])
    client.sys.enable_secrets_engine(
        backend_type="kv", options={"version": version}, path=path
    )
